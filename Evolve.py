...
GEMINI_MODELS = [
    "gemini-3.1-flash-lite-preview",  # Fastest + cheapest: ~382 t/s, $0.25/$1.50 per 1M
    "gemini-3-flash",
    "gemini-2.0-flash",                  # Best balance: frontier reasoning, $0.50/$3.00 per 1M
    "gemini-2.5-flash",        # Best balance: fast + capable, ~300 RPM on Tier 1
    "gemini-2.5-flash-lite",   # Highest throughput, most budget-friendly
    "gemini-2.5-pro",          # Most capable, ~150 RPM on Tier 1
]
...
def run_pacman_game(agent_fn, map_idx: int = 0,
                    max_steps: int = 200, seed: int = 42) -> Dict:
    """Run a single Pacman game given a pre-compiled agent callable."""
...
def evaluate_pacman(agent_code: str, runs: int = 2) -> Tuple[float,float,float]:
    # Validate and compile once — not once per game run (major runtime fix).
    ...
    for seed in range(runs):
        for map_idx in range(len(PACMAN_MAPS)):
            res = run_pacman_game(agent_fn, map_idx=map_idx, seed=seed)
            if res["error"]:
...
def run_evolution(
    ...
    # ── NO_EVOLUTION: single-shot — one LLM call before any generation loop ──
    # The result is frozen and reused verbatim in every generation.
    _no_evo_code, _no_evo_op = single_shot_llm(
        initial_code, problem, api_key, problem_type, model_name)
    log("[NO-EVOLUTION] Single-shot code generated once: {}".format(_no_evo_op))
        ...
        for strategy in active_strategies:
            ...
            for _trial in range(n_trials):
                ...
                if strategy == NO_EVOLUTION:
                    # Single-shot: always use the code generated once before the loop.
                    # Never re-call the LLM — this is the frozen baseline.
                    candidate, op = _no_evo_code, _no_evo_op
                    ...
                    # Cache key includes gen so each generation gets a fresh LLM call.
                    # Using only the parent hash caused the same result to be reused
                    # every generation after gen 1, breaking the evolution curve.
                    llm_cache_key = "{}_gen{}".format(h, gen)
                    if llm_cache_key in st.session_state.cache and \
                       "llm_code" in st.session_state.cache[llm_cache_key]:
                        candidate = st.session_state.cache[llm_cache_key]["llm_code"]
                        op = "[CACHE HIT] Reused prior LLM mutation"
                        log("  [LLM] Cache hit for code hash {} gen {}".format(h, gen))
                    elif api_key.strip():
                        try:
                            candidate, op = llm_mutate_real(
                                parent, problem, gen, api_key,
                                problem_type, ctx, model_name)
                            st.session_state.cache.setdefault(llm_cache_key, {})["llm_code"] = candidate
        ...
        for gen in range(1, n_gen+1):
            ...
            for _trial in range(n_trials):
                ...
                if strategy == NO_EVOLUTION:
                    ...
                    llm_cache_key = "{}_gen{}".format(h, gen)
                    if llm_cache_key in cache and "llm_code" in cache[llm_cache_key]:
                        candidate = cache[llm_cache_key]["llm_code"]; op = "cache_hit"
                    elif api_key.strip():
                        try:
                            candidate, op = llm_mutate_real(
                                parent, problem, gen, api_key, problem_type, ctx, model_name)
                            cache.setdefault(llm_cache_key, {})["llm_code"] = candidate
                        except Exception:
                            # FIX: pass problem_type
                            candidate, op = llm_mutate_demo(parent, gen, ctx, problem_type)
                    else:
